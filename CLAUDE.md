...
```json
"windows": {
  "installMode": "currentUser"  // upstream: "passive"
}
```
**Why**: Updater uses per-user install mode to match the NSIS installer.

### 3. `.github/workflows/release.yml`
Complete rewrite from upstream. Key differences:
- **Removed**: macOS builds, Linux builds (amd64/arm64/armv7), Fixed WebView2 builds, Winget submission, Telegram notification
...
| `tauri.conf.json` | Keep `"currentUser"` in updater.windows, accept upstream version/plugin updates |
...
- **`core/`** — Core business logic: handle (global state), hotkey, autostart, tray, updater, sysopt, service, notification
